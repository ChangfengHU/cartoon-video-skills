...
用户明确不需要 A/B 试听产品或候选选择步骤，优先希望自主获取合适的抖音热门配乐，也认可已有下班小人/红衣的配乐效果。因此前述试听页方案不再作为必做开发或验收项，game-audio-kit 只保留为用户日后明确要求比较时的参考。

本轮已修改 studio-music 入口和下班小人引用文档：自主选曲；优先实际抖音线索、以叙事适配决策；无强制候选数量或 A/B；保留内部听审、完整混音检查、真实来源和使用范围；各素材源接入状态须如实报告。对应插件镜像与本机已安装的这两个文件同步，先保存本地备份。

验证：skill quick_validate 与 studio validator 通过（18 skills、5 MCP、426 upstream files 未改变）。本轮是规则修改，没有新增搜索下载 MCP、实际选曲试听、新成片或 Fleet 部署；语法验证不代表音乐质量验证。
